#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════
#  HODGE FLOW — CHESS LABORATORY — one-command GitHub publisher
#  Works in: Android (Termux), Linux, macOS
#  Repository: hodge-flow-chess
...
#    3) creates the hodge-flow-chess repository via the GitHub API;
#    4) pushes the code;
...
REPO_NAME="${REPO_NAME:-hodge-flow-chess}"
GH_ACCOUNT="${GH_ACCOUNT:-wild8highlander}"
REPO_DESC="Hodge Laboratory Web — the Dynamic Principle in the browser: real-time chessboard flow modeling: t* = lcm(W/gcd(a,W), H/gcd(b,H)), μ₄ terminal cycles, K3 reflections + Klein Singer-7, equivariance grid, towers, 600 dpi reports. Bilingual RU/EN. Author: Isaev Iskhak Khamzatovich."
...
git add -A
git commit -m "hodge-flow-chess: real-time chessboard flow of the Dynamic Principle (t* = lcm, μ₄-orbits, K3/Klein layers, towers)" >/dev/null 2>&1 || okl "nothing new to commit"
